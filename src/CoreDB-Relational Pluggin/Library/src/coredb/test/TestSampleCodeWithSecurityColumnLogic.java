 ...
 * @author vmc
 */
public class TestSampleCodeWithSecurityColumnLogic {

	private  EntityControllerSecurity ec                = null;
	...
	public static void  main(String[] args) throws Exception{
		TestSampleCodeWithSecurityColumnLogic test = new TestSampleCodeWithSecurityColumnLogic();	
		test.setUp();
		// Case 1DENYGROUP
	...
	private void testColumnSecurity1() throws Exception {
		...
		/**
		 * bug, createReusltInstances is empty
		 */
		//Assert.assertFalse(TestAssist.compare(ec.getDatabaseConnection(), createReusltInstances, InstancesSet));
		...
		// Create - user 2	
		Assert.assertTrue(createReusltInstances.isEmpty());
		
		// Read - user 2	
		for(EntityInstance readReadInstance : readResultInstances) {
			Assert.assertEquals(readReadInstance.get(TESTTABLE_COLUMN_PHONE).getClass(), UnreadableValue.class );
			Assert.assertEquals(readReadInstance.get(TESTTABLE_COLUMN_NAME).getClass(), UnreadableValue.class );
		}
		
		// Update - user 2
		Assert.assertTrue(updateResultInstances.isEmpty());
		
		// Delete - user 2
		Assert.assertTrue(!deleteResultInstances.isEmpty());
	...
	private void testColumnSecurity2() throws Exception {
		...
		// Read - user1
		for(EntityInstance readReadInstance : readResultInstances) Assert.assertEquals(readReadInstance.get(TESTTABLE_COLUMN_PHONE).getClass(), UnreadableValue.class );
	...
	private void testColumnSecurity4() throws Exception {
		...
		List<EntityInstance> tokens1 = new LinkedList<EntityInstance>();
		tokens1.add(TOKEN_DENYGROUP$U$D);
		tokens1.add(TOKEN_DENYGROUP$C$D);
		tokens1.add(TOKEN_DENYGROUP$R$D);
		tokens1.add(TOKEN_DENYGROUP$D$D);
		...
		int i = 0;
		for(EntityInstance readResultInstance : readResultInstances){
			Assert.assertEquals(InstancesSet.get(i).get(TESTTABLE_COLUMN_PHONE), readResultInstance.get(TESTTABLE_COLUMN_PHONE));
			Assert.assertEquals(readResultInstance.get(TESTTABLE_COLUMN_NAME).getClass() , UnreadableValue.class);
		}
	 ...
	 *     				  |                        /       \         \			\
	 *     				  |                       /         \         \			 \
	 *     			    user2                coredbid       UserId    UserName   UserPass
	 *                  /   \                  /            /          /            /
	...
	private void testColumnSecurity5() throws Exception {
		...
		// Read - user 2	
		Assert.assertTrue(readResultInstances.isEmpty());
	...
	private HashMap<Character, List<EntityInstance>> doCRUD(EntityInstance user) {
		...
		curdSampleAndResultMap.put(CRUD.READ, resultInstances);
		printTag("\n[Result]\n");
		...
		EntityInstance sampleInstance4 = returnRealResult(sampleInstance0).get(0);
		sampleInstance4.set(TESTTABLE_COLUMN_NAME, "Super" + sampleInstance4.get(TESTTABLE_COLUMN_NAME));	
		EntityInstance entityInstance5 = sampleInstance4.clone();
		sampleInstances.add(entityInstance5);
		resultInstances = ec.updateEntityWithSecurity(user, sampleInstance4).getPassedEntities();
		if(resultInstances.size() > 0) {
			...
			curdSampleAndResultMap.put(CRUD.UPDATE, returnRealResult(entityInstance5));
		}
		...
		printTag("\n[Before Delete]\n");
		Describer.describeTable((IEntityControllerCommon)ec, TESTTABLE_TABLENAME, "");
		EntityInstance sampleInstance5 = returnRealResult(sampleInstance0).get(0);
		ec.deleteEntityWithSecurity(user, sampleInstance5).getPassedEntities();
...
//				if(entity.get(attribute.getName()) instanceof UnreadableValue){
//					sql    = entity.makeSQLOnIdentifiableAttributes(ec.getDatabaseConnection());
//					entity = ec.getDatabaseConnection().read(entity.getDynaClass().getName(), sql).get(0);
//					break;
//				}
//			}
			
			sql = entity.makeSQLOnColumn(ec.getDatabaseConnection(), TESTTABLE_COLUMN_NAME, Q.E);
			sql += Q.AND;
	...
	private void createTokens() {
		...
		TOKEN_DENYGROUP$C$D.set(Token_Info_Table.getColumnTokenID(), "userdenygroup$C$D");
		TOKEN_DENYGROUP$C$D.set(Token_Info_Table.getColumnTokenName(), "userdenygroup$C$D");
		TOKEN_DENYGROUP$R$D.set(Token_Info_Table.getColumnGroup(), DENYGROUP);
		TOKEN_DENYGROUP$R$D.set(Token_Info_Table.getColumnTokenID(), "userdenygroup$R$D");
		TOKEN_DENYGROUP$R$D.set(Token_Info_Table.getColumnTokenName(), "userdenygroup$R$D");
		TOKEN_DENYGROUP$U$D.set(Token_Info_Table.getColumnGroup(), DENYGROUP);
		TOKEN_DENYGROUP$U$D.set(Token_Info_Table.getColumnTokenID(), "userdenygroup$U$D");
		TOKEN_DENYGROUP$U$D.set(Token_Info_Table.getColumnTokenName(), "userdenygroup$U$D");
		TOKEN_DENYGROUP$D$D.set(Token_Info_Table.getColumnGroup(), DENYGROUP);
		TOKEN_DENYGROUP$D$D.set(Token_Info_Table.getColumnTokenID(), "userdenygroup$D$D");
		TOKEN_DENYGROUP$D$D.set(Token_Info_Table.getColumnTokenName(), "userdenygroup$D$D");
