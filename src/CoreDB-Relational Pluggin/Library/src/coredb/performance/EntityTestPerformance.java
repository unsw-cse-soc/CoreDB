 ...
 * 
 */
class EntityTestPerformance {
    static int start = 0;
    static int end = 100;

	/**
	 * create 100 student records and do CRUD processes on it for testing speed
	 * 
	...
	public static void benchmark(IEntityControllerBase iec, boolean security, boolean tracing) {
        ...
        if (security && tracing) /** VMC: TO BE IMPLEMENTED */;
        else if (security)  student_index = new EntitySecurityClass(PERFORMANCE_STUDENT_INDEX, STU_acl_index.toArray(new AttributeClass[0]), null);
        else if (tracing)   student_index = new EntityTracingClass(PERFORMANCE_STUDENT_INDEX, STU_acl_index.toArray(new AttributeClass[0]), null);
        else                student_index = new EntityClass(PERFORMANCE_STUDENT_INDEX, STU_acl_index.toArray(new AttributeClass[0]), null);
        ...
        if (security && tracing) /** VMC: TO BE IMPLEMENTED */;
        else if (security)  student_index_2 = new EntitySecurityClass(PERFORMANCE_STUDENT_INDEX_2, STU_acl_index_2.toArray(new AttributeClass[0]), null);
        else if (tracing)   student_index_2 = new EntityTracingClass(PERFORMANCE_STUDENT_INDEX_2, STU_acl_index_2.toArray(new AttributeClass[0]), null);
        else                student_index_2 = new EntityClass(PERFORMANCE_STUDENT_INDEX_2, STU_acl_index_2.toArray(new AttributeClass[0]), null);
...
//DELETE
    ...
    private static void multipleSingleReads(IEntityControllerBase iec, String tableName) {
    	...
    	for(int id = start; id < end; id ++) {
            ...
            long diff_end_time = System.currentTimeMillis();
            package_time += (diff_end_time - diff_start_time);
            iec.readEntities(tableName, new ConditionalStatement(new AttributeClass("ID",Integer.class),Operation.E,id+"")).get(0);
    	}
    	long end_time = System.currentTimeMillis();
        long total_time_taken = end_time - start_time;
    ...
    private static void multipleSingleUpdates(IEntityControllerBase iec, String tableName) {
    	...
    	for(int id = start; id < end; id ++)
    	{
            student = iec.readEntities(tableName, new ConditionalStatement(new AttributeClass("ID".toLowerCase(),Integer.class,TypeMap.INTEGER),"=",id+"")).get(0);
    		student.set("ID".toLowerCase(), id);
			student.set("PASSPORT".toLowerCase(), id);
    ...
    /**
     * create 100 student records and do CRUD processes on it for testing sql statement speed
     * 
     * @param iec IEntityControllerBase
